...
def get_graph():
    ...
    results = db.read_transaction(lambda tx: list(tx.run("MATCH (m:Movie)<-[:ACTED_IN]-(a:Person) "
                                                         "RETURN m.title as movie, collect(a.name) as cast, count(a) as weight "
                                                         "LIMIT $limit", {
    ...
    for record in results:
        nodes.append({"title": record["movie"], "label": "movie", "weight": record["weight"]})
        target = i
        i += 1
        for name in record['cast']:
            actor = {"title": name, "label": "actor", "weight": record["weight"]}
            try:
            ...
            rels.append({"source": source, "target": target, "weight": 2})
